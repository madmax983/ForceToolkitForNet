...
{
    public class ToolingClient : IToolingClient, IDisposable
    {
        private readonly ServiceHttpClient _serviceHttpClient;

        public ToolingClient(string instanceUrl, string accessToken, string apiVersion)
            : this(instanceUrl, accessToken, apiVersion, new HttpClient())
        {
        }

        public ToolingClient(string instanceUrl, string accessToken, string apiVersion, HttpClient httpClient)
        {
        ...
        public Task<T> CompletionsAsync<T>(string type)
        {
            if (string.IsNullOrEmpty(type)) throw new ArgumentNullException("completions");

            return _serviceHttpClient.HttpGetAsync<T>(string.Format("tooling/completions?type={0}", type));
        }

        public Task<T> ExecuteAnonymousAsync<T>(string body)
        {
            if (string.IsNullOrEmpty(body)) throw new ArgumentNullException("body");

            return _serviceHttpClient.HttpGetAsync<T>(string.Format("tooling/executeAnonymous/?anonymousBody={0}", body));
        }

        public Task<T> RunTestsAsynchronousAsync<T>(string classids)
        {
            if (string.IsNullOrEmpty(classids)) throw new ArgumentNullException("classids");

            return _serviceHttpClient.HttpGetAsync<T>(string.Format("tooling/runTestsAsynchronous/?classids={0}", classids));
        }
        ...
        public Task<QueryResult<T>> QueryAsync<T>(string query)
        {
            ...
            return _serviceHttpClient.HttpGetAsync<QueryResult<T>>(string.Format("tooling/query?q={0}", query));
        }
        ...
        public async Task<string> CreateAsync(string objectName, object record)
        {
            ...
            var response = await _serviceHttpClient.HttpPostAsync<SuccessResponse>(record, string.Format("tooling/sobjects/{0}", objectName)).ConfigureAwait(false);
            return response.id;
        }
        ...
        public Task<SuccessResponse> UpdateAsync(string objectName, string recordId, object record)
        {
            ...
            return _serviceHttpClient.HttpPatchAsync(record, string.Format("tooling/sobjects/{0}/{1}", objectName, recordId));
        }
        ...
        public Task<SuccessResponse> UpsertExternalAsync(string objectName, string externalFieldName, string externalId, object record)
        {
            ...
            return _serviceHttpClient.HttpPatchAsync(record, string.Format("tooling/sobjects/{0}/{1}/{2}", objectName, externalFieldName, externalId));
        }
        ...
        public Task<bool> DeleteAsync(string objectName, string recordId)
        {
            ...
            return _serviceHttpClient.HttpDeleteAsync(string.Format("tooling/sobjects/{0}/{1}", objectName, recordId));
        }

        public Task<DescribeGlobalResult<T>> GetObjectsAsync<T>()
        {
            return _serviceHttpClient.HttpGetAsync<DescribeGlobalResult<T>>("tooling/sobjects");
        }
        ...
        public Task<T> BasicInformationAsync<T>(string objectName)
        {
            ...
            return _serviceHttpClient.HttpGetAsync<T>(string.Format("tooling/sobjects/{0}", objectName));
        }
        ...
        public Task<T> DescribeAsync<T>(string objectName)
        {
            ...
            return _serviceHttpClient.HttpGetAsync<T>(string.Format("tooling/sobjects/{0}/describe/", objectName));
        }
        ...
        public Task<T> GetDeleted<T>(string objectName, DateTime startDateTime, DateTime endDateTime)
        {
            ...
            return _serviceHttpClient.HttpGetAsync<T>(string.Format("tooling/sobjects/{0}/deleted/?start={1}&end={2}", objectName, sdt, edt));
        }
        ...
        public Task<T> GetUpdated<T>(string objectName, DateTime startDateTime, DateTime endDateTime)
        {
            ...
            return _serviceHttpClient.HttpGetAsync<T>(string.Format("tooling/sobjects/{0}/updated/?start={1}&end={2}", objectName, sdt, edt));
        }
